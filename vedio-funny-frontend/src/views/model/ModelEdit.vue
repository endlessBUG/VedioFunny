...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      // 这里可以根据 route.params.id 拉取模型详情并回显
      // 模拟加载LoRA模型数据
      const modelId = route.params.id
      if (modelId === '5') { // 假设ID为5的是LoRA模型
        form.name = 'Anime-Style-LoRA'
        form.category = 'lora'
        form.version = '1.2'
        form.provider = 'CivitAI'
        form.size = '144MB'
        form.baseModel = 'Stable Diffusion 1.5'
        form.triggerWords = ['anime style', 'manga']
        form.recommendedStrength = 0.8
        form.accuracy = 89
        form.status = 'active'
        form.description = '动漫风格LoRA模型，适用于Stable Diffusion'
        ...
        form.accuracy = 95
        form.status = 'active'
        form.description = '这是一个示例模型'
        form.config = '{"maxTokens":4096,"temperature":0.7}'
      }
    })
